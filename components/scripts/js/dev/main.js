...
function initExtra(topWindow) {
  ...
  // today.setHours(0,0,0,0);
   // var trialEnd = new Date("12-10-2020");
   // if (trialEnd <= today) {
  ...
  _extra.registerModule = function(
    ...
    function registerSubordinate(moduleName, subordinateName) {
      moduleRegistry[moduleName].subordinates[subordinateName] =
        moduleRegistry[subordinateName];
    ...
    if (!moduleRegistry[moduleName]) {
      ...
      throw Error(
        "Tried to register two modules under the name: " + moduleName
      );
    }
